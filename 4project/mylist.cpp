...
bool ListInit(List* list, int size) {
...
int UsageSize(List* list) { // the size that is used

    CHECKLIST(list);

    int size = 0;
    for(int i = list->head; i != 0; i = list->Elem[i].next, size++)
...
int PopFirst(List* list) { //pop head of the list
    ...
    int value = list->Elem[list->head].data;

    int exhead = list->head;
    list->head = list->Elem[list->head].next;
    list->Elem[list->head].prev = 0;
    int exfree = list->free;

    list->tail = (list->head == 0) ? 0 : list->tail;

    list->free = exhead;
    list->Elem[list->free].next = exfree;
    list->Elem[list->free].prev = -1;
    list->Elem[list->free].data = POISON;

    CHECKLIST(list);
    return value;
}

int ListFill(List* list, Elem_t min, int amount, Elem_t step) {
    CHECKLIST(list);

    for(int count = 0; amount > 0; amount--, count++) {
        PushLast(list,  min + count * step);
    }
    CHECKLIST(list);
